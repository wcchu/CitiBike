...
tmp = data.frame(hi = round(as.integer(strftime(master$ti, format = "%H")) +
                            as.integer(strftime(master$ti, format = "%M")) / 60, 2),
                 di = as.integer(strftime(master$ti, format = "%u")))
...
```{r, echo = F}
tmp = data.frame(hf = round(as.integer(strftime(master$tf, format = "%H")) +
                            as.integer(strftime(master$tf, format = "%M")) / 60, 2),
                 df = as.integer(strftime(master$tf, format = "%u")))
...
```{r, echo = F}
tmp = left_join(master, stations, by = c('si' = 'id')) %>% select(lat, lon) %>% sample_n(5000)
...
```{r, echo = F}
tmp = left_join(master, stations, by = c('sf' = 'id')) %>% select(lat, lon) %>% sample_n(5000)
fig = ggmap(nycmap, extent = "device") +
  stat_density2d(data = tmp, aes(x = lon, y = lat, fill = ..level.., alpha = ..level..),
	size = 0.01, bins = 16, geom = "polygon") +
	geom_density2d(data = tmp, aes(x = lon, y = lat), size = 0.3) +
