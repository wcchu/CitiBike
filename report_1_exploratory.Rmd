---
title: 'CitiBike rental system Part 1: Exploratory data analysis'
author: "Wei-Chun Chu"
...
# 1. Data structure and basic system profile

First we load the csv file into R as the master dataset 'citibike'. The dataset records all CitiBike rides in July 2014. Its column names are renamed for simplicity.

```{r}
set.seed(123456)
library(dplyr) # in the whole project we use dplyr package to manipulate datasets.
citibike = read.csv('citibike_2014-07.csv', header = T)
colnames(citibike) = c("dur", "ti", "tf",
                       "si", "namei", "lati", "loni",
                       "sf", "namef", "latf", "lonf",
                       "bike", "type", "birth", "sex")
str(citibike)
...
```{r}
n_bikes = n_distinct(citibike$bike) # total number of bikes
n_sta = n_distinct(citibike$si) # total number of stations
...
tmp = data.frame(hi = round(as.integer(strftime(citibike$ti, format = "%H")) +
                            as.integer(strftime(citibike$ti, format = "%M")) / 60, 2),
                 di = as.integer(strftime(citibike$ti, format = "%u")))
...
```{r, echo = F}
tmp = data.frame(hf = round(as.integer(strftime(citibike$tf, format = "%H")) +
                            as.integer(strftime(citibike$tf, format = "%M")) / 60, 2),
                 df = as.integer(strftime(citibike$tf, format = "%u")))
tmp = tmp %>% sample_n(5000)
fig = ggplot() +
...
Now we create the density plots for the start and end locations. A .
...
```{r, echo = F}
# google map with matching range is called by ggmap as the background of the figures
...
# 2d density
tmp = citibike %>% select(lat = lati, lon = loni) %>% sample_n(5000)
